...
export class RegisterComponent implements OnInit {
  ...
  passwordMatchValidator(g: FormGroup) {
    return g.get("artist_password").value === g.get("artist_conf").value
      ? null
      : g.get("artist_conf").setErrors({ mismatch: true });
  }
  ...
  getLocation() {
    if (this.artist_data["zipcode"]) {
      this.registerService
        .getLocationFromZipCode(this.artist_data["zipcode"])
        .subscribe((response) => {
          const res = response;
          if (
            res["results"].length > 0 &&
            res["results"][0].hasOwnProperty("address_components")
          ) {
            /***************************************** old code ***********************************/
            ...
            for (
              let i = 0;
              i < res["results"][0].address_components.length;
              i++
            ) {
              const addressType =
                res["results"][0].address_components[i].types[0];
  ...
  getLocationForListener() {
    if (this.listener_data["zipcode"]) {
      this.registerService
        .getLocationFromZipCode(this.listener_data["zipcode"])
        .subscribe((response) => {
          const res = response;
          if (
            res["results"].length > 0 &&
  ...
  artist_submit() {
    ...
    formData.append(
      "dob",
      new Date(
        this.artist_data["year"],
        this.artist_data["month"],
        this.artist_data["day"]
      ).toString()
    );

    this.show_spinner = true;
    this.registerService.artistRegistration(formData).subscribe(
      (response) => {
        this.step_flag = true;
        this.location = "";
        this.artist_cnt = 0;
        ...
        this.toastr.success(
          "Registration done successfully and confirmation email sent to your account please verify to do login.",
          "Success!"
        );
  ...
  listener_submit() {
    if (
      this.listener_data.music_type &&
      this.listener_data.music_type.length <= 0
    ) {
      ...
      const data = {
        ...
        dob: new Date(
          this.listener_data["year"],
          this.listener_data["month"],
          this.listener_data["day"]
        ),
      };
      this.show_spinner = true;
      this.registerService.listenerRegistration(data).subscribe(
        (response) => {
          this.step_flag = true;
          this.listner_cnt = 0;
          this.location = "";
  ...
  fbLogin() {
    ...
    this.socialAuthService.signIn(socialPlatformProvider).then((data) => {
      ...
      this.registerService.userFacebookLogin(data).subscribe(
        (response) => {
          this.toastr.success(response["message"], "Success!");
          localStorage.setItem("user", JSON.stringify(response));
          this.user = JSON.parse(localStorage.getItem("user"));
