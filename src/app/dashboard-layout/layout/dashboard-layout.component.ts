...
export class DashboardLayoutComponent
  ...
  play() {
    try {
      ...
      console.log(" : audio_ins ==> ", this.audio_ins);
      ...
      if (
        this.audio_instance_list[this.song_cnt] &&
        this.audio_instance_list[this.song_cnt].paused
      ) {
        ...
        if (playPromise !== undefined) {
          playPromise
            ...
            .catch((error) => {
              this.toastr.info(
                "This audio type is not supported in this browser.",
                "Information!"
              );
  ...
  timeUpdate($event: any) {
    ...
    const str =
      ...
      ":" +
      (isNaN(seconds) ? 0 : Math.round(seconds));
  ...
  downloadTrack() {
    if (this.user) {
      this.dashboardLayoutService
        .downloadTrack(this.audio_list[this.song_cnt]["_id"])
        .subscribe((response) => {
          if (response["message"]) {
  ...
  share_via_email(flag: boolean) {
    if (flag) {
      ...
      this.dashboardLayoutService.shareTrackViaEmail(data).subscribe(
        (response) => {
  ...
  share_via_sms(flag: boolean) {
    if (flag) {
      ...
      this.dashboardLayoutService.shareTrackViaSms(data).subscribe(
        (response) => {
          this.toastr.success(response["message"], "Success!");
          this.emailmodalRef.close();
          this.share_data = {};
