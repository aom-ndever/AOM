...
export class VoteService {
  ...
  followArtist(data: any) {
    this.user = JSON.parse(localStorage.getItem("user"));
    this.headers = new HttpHeaders({ "x-access-token": this.user.token });
    return this.http.post(`${this.api_host}/user/artist/follow`, data, {
  ...
  giveVoteToTrack(data) {
    this.user = JSON.parse(localStorage.getItem("user"));
    this.headers = new HttpHeaders({ "x-access-token": this.user.token });
    return this.http.post(`${this.api_host}/user/track/vote_track`, data, {
      headers: this.headers,
    });
  }
  // Get all contest
  ...
  getFollower() {
    this.user = JSON.parse(localStorage.getItem("user"));
    this.headers = new HttpHeaders({ "x-access-token": this.user.token });
    return this.http.get(`${this.api_host}/user/artist/followed`, {
      headers: this.headers,
    });
  }
}
