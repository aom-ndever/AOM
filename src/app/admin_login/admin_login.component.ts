...
export class AdminLoginComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.user && this.user.hasOwnProperty("admin")) {
  ...
  signin() {
    ...
    this.adminLoginService.login(this.admin_data).subscribe(
      (response) => {
  ...
  forgetPassword(flag) {
    if (flag) {
      this.show_spinner = true;
      this.adminLoginService.forgetPassword(this.forget_data).subscribe(
        (response) => {
          this.forget_data = {};
          this.ModelRef.hide();
