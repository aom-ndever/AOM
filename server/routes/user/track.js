      ...
      if (resp_data.status == 0) {
        ...
        if (card_resp.status == 0) {
          ...
          res
            ...
            .json({ message: "Artist doesn't have bank account" });
        }
...
router.post("/purchased", async (req, res) => {
  ...
  var resp_data = await purchase_helper.get_purchased_track(
    user_id,
    req.body.start,
    req.body.length
  );
...
router.post("/vote_track", async (req, res) => {
  ...
  user_id = req.userInfo.id;
  var data = await vote_track_helper.get_all_voted_artist(
    user_id,
    obj.track_id,
    obj.contest_id
  );
  ...
  if (!errors) {
    ...
    if (resp_data && resp_data.like == 0) {
      ...
      if (data && data.status == 0) {
        ...
        var resp_data = await track_helper.get_all_track_by_track_id(
          obj.track_id
        );
      ...
      var resp_data = await user_helper.update_user_for_likes(
        obj.user_id,
        no_like
      );
      ...
      res
        ...
        .json({ message: "Like has been done", flag: "liked" });
    } else {
      ...
      res
        .status(config.OK_STATUS)
        .json({ message: "Like has been removed", flag: "unliked" });
    }
  } else {
    logger.error("Validation Error = ", errors);
    res.status(config.BAD_REQUEST).json({ message: errors });
...
router.post("/share_track_by_sms", async (req, res) => {
  ...
  try {
    ...
    if (msg) {
      res
        .status(config.OK_STATUS)
        .json({ message: "Track shared successfully" });
    } else {
      res
        .status(config.OK_STATUS)
        .json({ message: "Your number is not registered" });
    }
  } catch (error) {
    res
      .status(config.OK_STATUS)
...
router.get("/:track_id/download", async (req, res) => {
  try {
    ...
    if (resp.track.is_downloadable == false) {
      res
        .status(200)
        .json({ status: 0, message: "track cannot be downloaded" });
    } else {
      ...
      if (resp_data.status == 0) {
        ...
        var resp_data = await download_helper.get_all_track_by_track_id(
          obj.track_id
        );
        no_download = resp_data.track.no_of_downloads + 1;
        var resp_data = await download_helper.update_downloads(
          obj.track_id,
          no_download
        );
        if (resp_data.status == 1) {
          logger.trace("music got successfully = ", resp_data);
          let track_resp = await track_helper.get_all_track_by_track_id(
            req.params.track_id
          );
          if (track_resp.status == 1) {
            ...
            var output = await fs.createWriteStream(
              __dirname + "/../../uploads/user/" + filename
            );
            ...
            archive.append(
              fs.createReadStream(
                __dirname + "/../../uploads/track/" + track_resp.track.audio
              ),
              { name: track_resp.track.audio }
            );
            archive.finalize();
