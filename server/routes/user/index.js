...
router.post("/upgrade_to_artist", async (req, res) => {
  ...
  if (!errors) {
    ...
    let artist = await artist_helper.get_artist_by_email(resp_data.user.email);
    if (artist.status === 2) {
      var obj = {};
      //image upload
      var filename;
      ...
      if (data.status == 0) {
        ...
        res
          ...
          .json({ status: 0, message: "You have been Upgraded to Artist" });
      }
    } else {
      res
        .status(config.BAD_REQUEST)
        .json({ status: 0, message: "Artist's email already exist" });
    }
  } else {
...
router.delete("/image/:user_id", async (req, res) => {
  ...
  if (del_resp.status === 0) {
    res.status(config.INTERNAL_SERVER_ERROR).json({
      status: 0,
      message: "Error occured while deleting user image",
      error: del_resp.error,
    });
  } else if (del_resp.status === 2) {
    res
      .status(config.BAD_REQUEST)
      .json({ status: 0, message: "Can't delete user image" });
  } else {
    res
      .status(config.OK_STATUS)
      .json({ status: 1, message: "User image has been deleted" });
  }
});
    ...
    if (resp.status === 1) {
      if (resp.user.email == req.body.email) {
        if (req.body.new_email) {
          var resp = await user_helper.update_user_email(
            user_id,
            req.body.new_email
          );
          res
            .status(config.OK_STATUS)
            .json({ status: 1, resp: "Email changed" });
        } else {
          res
            .status(config.OK_STATUS)
  ...
  if (resp.status === 1) {
    if (bcrypt.compareSync(req.body.password, resp.user.password)) {
      if (req.body.new_password) {
        var resp = await user_helper.update_user_password(user_id, {
        ...
        res
          .status(config.OK_STATUS)
          .json({ status: 1, resp: "Please Enter New Password" });
      }
    } else {
      res
        .status(config.BAD_REQUEST)
