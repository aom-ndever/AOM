...
router.post("/follow", async (req, res) => {
  ...
  if (!errors) {
    ...
    var resp_datas = await follower_helper.get_all_follows(
      obj.artist_id,
      obj.user_id
    );
    if (resp_datas && resp_datas.user == 0) {
      ...
      var resp_data = await track_helper.update_artist_for_followers(
        obj.artist_id,
        no_follow
      );
      ...
      var resp_data = await user_helper.update_user_for_followers(
        obj.user_id,
        no_follow
      );
      ...
      res
        ...
        .json({ message: "You Have followed", flag: "follow" });
      // }
    } else {
      var resp_data = await follower_helper.delete_follow(
        obj.artist_id,
        obj.user_id
      );
      var resp = await artist_helper.get_artist_by_id(obj.artist_id);
      ...
      res
        .status(config.OK_STATUS)
        .json({ message: "You Have Unfollowed", flag: "unfollow" });
    }
  } else {
