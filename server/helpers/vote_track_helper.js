...
vote_track_helper.get_all_voted_artist = async (
  user_id,
  track_id,
  contest_id
) => {
  try {
    var vote = await Vote.findOne({
      ...
      contest_id: new ObjectId(contest_id),
    });
    if (vote) {
...
vote_track_helper.get_all_voted_artist_by_id = async (artist_id) => {
  try {
    var vote = await Vote.find({ artist_id: new ObjectId(artist_id) })
