...
user_helper.get_user_by_id = async (user_id) => {
  try {
    var user = await User.findOne({ _id: new ObjectId(user_id) }).populate(
      "music_type"
    );
...
user_helper.update_user_by_id = async (user_id, object) => {
  try {
    ...
    if (!user) {
      ...
      let user_data = await User.findOne({ _id: user_id }).populate(
        "music_type"
      );
      return {
        status: 1,
...
user_helper.update_user_status = async (user_id, status) => {
  try {
    ...
    return {
      status: 0,
      message: "Error occured while updating user status",
      error: err,
    };
  }
};
...
user_helper.update_user_for_votes = async (id, no_vote) => {
  try {
    var contest = await User.findOneAndUpdate(
      { _id: new ObjectId(id) },
      { no_of_votes: no_vote }
    );
    if (contest) {
...
user_helper.update_user_for_likes = async (id, no_like) => {
  try {
    ...
    return {
      status: 0,
      message: "Error occured while finding likes",
      error: err,
    };
  }
};
...
user_helper.get_all_active_and_suspend_user = async (
  start,
  length,
  filter,
  sort_by = {}
...
user_helper.update_user_email = async (artist_id, email) => {
  try {
    ...
    return {
      status: 0,
      message: "Error occured while updating user'email",
      error: err,
    };
  }
};
user_helper.update_user_password = async (user_id, password) => {
  try {
    ...
    return {
      status: 0,
      message: "Error occured while updating user's password",
      error: err,
    };
  }
};
