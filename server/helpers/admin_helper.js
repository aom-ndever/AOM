...
admin_helper.update_admin_request_by_adminid = async (id, type) => {
  try {
    var admin = await AdminRequest.findByIdAndUpdate(
...
admin_helper.get_all_admin = async (start, length, sort = {}) => {
  try {
    ...
    var admin = await Admin.find().sort(sort).skip(start).limit(length);
...
admin_helper.get_all_admin_request = async (start, length, sort = {}) => {
  try {
    ...
    var admin = await AdminRequest.find().sort(sort).skip(start).limit(length);
...
admin_helper.get_login_by_email = async (email, password) => {
  try {
    ...
    var count = await CopyrightTrackNotifications.find({
      receivers: {
        $elemMatch: {
          receiver: admin._id,
          isSeen: 0,
...
admin_helper.update_admin_status = async (admin_id, status) => {
  try {
    ...
    return {
      status: 0,
      message: "Error occured while updating admin status",
      error: err,
    };
  }
};
