...
playlist_helper.insert_playlist = async (object) => {
  ...
  try {
    ...
    return {
      status: 0,
      message: "Error occured while inserting playlist",
      error: err,
    };
  }
};
...
playlist_helper.get_playlist_by_user_id = async (user_id, start, length) => {
  try {
    ...
    return {
      status: 0,
      message: "Error occured while finding playlist",
      error: err,
    };
  }
};
...
playlist_helper.update_playlist = async (user_id, playlist_id, obj) => {
  try {
    var playlist = await Playlist.findOneAndUpdate(
      { user_id: new ObjectId(user_id), _id: new ObjectId(playlist_id) },
      obj,
      { new: true }
    );
    if (playlist) {
...
playlist_helper.delete_playlist = async (user_id, playlist_id) => {
  try {
    var playlist = await Playlist.findOneAndRemove({
...
playlist_helper.delete_track_playlist = async (
  user_id,
  playlist_id,
  newData
) => {
  try {
    var playlist = await Playlist.findByIdAndUpdate(
      { user_id: new ObjectId(user_id), _id: new ObjectId(playlist_id) },
      { track_id: newData }
    );
