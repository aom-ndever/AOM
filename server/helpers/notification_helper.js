 ...
 */
artist_notifications_helper.get_notifications = async (userId, skip = false, limit = false, sort = false) => {
    try {
...
artist_notifications_helper.notification_seen = async (id, updateObject) => {
    try {
        let resp = await ArtistNotifications.updateMany(id, updateObject);
        if (!resp) {
            return {
                status: 2,
